...
export default function ScreenSelector({ screens, activeKey, onSelect, onCreate }: Props) {
  ...
  const handleCreate = () => {
    ...
    const key = newLabel.trim().toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9가-힣-]/g, '') || `screen-${Date.now()}`
    onCreate(key, newLabel.trim())
    setNewLabel('')
    setShowNew(false)
  }
      ...
      {showNew ? (
        <div style={{ display: 'flex', gap: 4, alignItems: 'center', padding: '0 8px', marginBottom: -2 }}>
          <input
            ...
            style={{ padding: '4px 8px', borderRadius: 4, border: '1px solid #ddd', fontSize: 12, width: 120 }}
            autoFocus
          />
